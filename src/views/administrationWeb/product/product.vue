        ...
        <van-tabs v-model="active"
            ...
            <van-tab :title="item.name"
                ...
                <van-pull-refresh v-model="isLoading"
                                  @refresh="onRefresh">
                    <div @click="$router.push(`/productDetail?id=${item2.id}`)"
                        ...
                        <van-card class="bg card"
                            ...
                            <div slot="num"> {{item2.showtime}}</div>
                        </van-card>
                    </div>
                </van-pull-refresh>
...
export default {
  ...
  methods: {
    // 获取列表
    getList (objNum) {
      this.getRequest('content/product', this.data).then(res => {
        if (res) {
          let data = res.data
    ...
    // 获取分类
    getData () {
      this.postRequest('content/product_category').then(res => {
        if (res) {
          let data = res.data
          if (data.status === 1) {
            this.tab = data.data
    ...
    tabClick (item) {
      ...
      this.$router.push(`/product?tab=${item}&cid=${id}`)
      this.data.cid = id
      let strObj = []
