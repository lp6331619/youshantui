<template>
    <div class="login">
        ...
        <div class="optimization bg mt15">
            <div class="title"><i class="iconfont icon-baobiao"></i>商贸推广报表
            </div>
            <van-row gutter="20"
                     class="topBox">
                <van-col span="12">
                ...
                <van-col span="12">
                    <div class="num"><strong>{{keyword.kw_nums}}</strong> 条</div>
                    全网关键词上线数
                </van-col>
            </van-row>
        </div>
                ...
                <tr v-for="(item,index) in keyword.data"
                    ...
                    <td class="itemBox"
                        ...
                        <table cellpadding="0"
                               cellspacing="0"
                               class="itemBox">
                            <tr v-for="(item2,index2) in item.infos"
                                :key='index2'>
                                <td class="item"
                                    width="100px">
                                    <div style="width:100px">
                                        {{format(item2.platform_id)}}
                                    </div>
                                </td>
                                <td class="item">
                                    收录 <span class="num"> {{item2.nums}}</span>条
                                    <span style="display:inline-block;max-width:100%;word-wrap:break-word;"
                                          v-if="item2.ranking_json.pos && item2.ranking_json.pos.length>0">
                                        第
                                        <span class="num"
...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      page.page = 1
      this.data.page = 1
      this.$router.push({ path: '/commerceList', query: page })
      this.keywordBox()
    },
    format (num) {
      let text = ''
    ...
    keywordBox () {
      ...
      this.getRequest('spread/business_day_report', data).then(res => {
        if (res) {
    ...
    pageBox (index) {
      ...
      this.$router.push({ path: '/commerceList', query: this.data })
      this.keywordBox()
    }
  }
